<h1>#🥇7/3~7/8 진행한 경기대 부트캠프 Spring Framework 수업#
<hr>
...
<br>
<p>– 요청 흐름 제어, Front Controller의 역할을 수행(공통작업 수행)
<p>– 개발자는 spring이 제어하지 않는 sub controller만 제작
<p>- web에 맞는 spring container 제작

0.	요청 파라미터 인코딩 생성 -> spring이 처리
1.	요청파라미터 추출(유효성 검사 백엔드&프론트엔드 2중검사)
	자동화 가능
2.	Model에서 service logic을 불러옴
3.	성공 or 실패 처리에 따라 상황에 맞는 view로 이동 -->자동화

<h3>SpringBoot</h3>
<p>-	여러 환경설정을 내장하고 있음
<p>-	사용자는 필요한 내용을 갖다 쓰거나 없을 경우 추가 가능
...
<h2>REST: Representational Status Transfer</h2>
<br>
<p>-	네트워크 상의 자원을 URI로 표현하고 데이터를 주고받는 방법에 대한 아키텍처
<p>-	구성 요소</p>
  <br>
<p>1.	자원: URL – ex) ~~~/users, ~~~~/boards/1
<p>2.	작업: Method – Method 종류가 다르면 URL이 같아도 작동함</p>
    <p>-i.	get: 자원 조회
    <p>-ii.	post: 자원 생성(등록)
    <p>-iii.	put: 자원 수정, patch: 자원 일부 수정
    <p>-iv.	delete: 자원 삭제</p>
<br>
3.	표현: 데이터 + 처리 상태
<p> ->	데이터: text, csv, tsv, xml, json......
<p> -> 처리 상태: http status</p>

<br>
<p>- Restful: Rest 아키텍처를 잘 준수한
<p>- Restful API: Restful하게 만든 서비스</p>

<br>
<h3>Spring Boot</h3>
<p>-	과도한 설정으로 인한 프레임워크를 사용한 개발의 어려움을 해결
<p>-	내장된 설정 (애노테이션 + 자바 소스 Config, xml 지양)
<p>-	boot dependency
<p>-	서버 내장
